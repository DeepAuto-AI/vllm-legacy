...
void reshape_and_cache_flash(
  ...
  int num_heads = key.size(1);
  int head_size = key.size(2);
  int block_size = key_cache_size_1; //k_cache.size(1);

  int key_stride = key.stride(0);
  int value_stride = value.stride(0);
  // int block_stride = k_cache.stride(0);
  int block_stride = key_cache_stride_0;
  // TORCH_CHECK(k_cache.stride(0) == v_cache.stride(0));
  TORCH_CHECK(key_cache_stride_0, value_cache_stride_0);
      ...
      key.scalar_type(), "reshape_and_cache_flash", [&] {
        vllm::reshape_and_cache_flash_kernel<scalar_t>
            <<<grid, block, 0, stream>>>(
                ...
                reinterpret_cast<scalar_t*>(key_cache_data_ptr),
                // v_cache.data_ptr<scalar_t>(),
                reinterpret_cast<scalar_t*>(value_cache_data_ptr),
